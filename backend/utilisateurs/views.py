...
def inscription(request):
    # Le rôle n'est plus accepté depuis le frontend : InscriptionSerializer
    # force toujours role='passager' dans sa méthode create().
...
def changer_mot_de_passe(request):
    ...
    if not ancien or not nouveau:
        return Response({
            'erreur': 'Ancien et nouveau mot de passe obligatoires'
        }, status=status.HTTP_400_BAD_REQUEST)
